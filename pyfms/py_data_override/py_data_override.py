...
class pyDataOverride():
    ...
    def data_override_init(self,
        ...
        if atm_domain_id is not None : atm_domain_id = atm_domain_id_t(atm_domain_id)
        if ocn_domain_id is not None : ocn_domain_id = ocn_domain_id_t(ocn_domain_id)
        if ice_domain_id is not None : ice_domain_id = ice_domain_id_t(ice_domain_id)
        if land_domain_id is not None : land_domain_id = land_domain_id_t(land_domain_id)
        if land_domainUG_id is not None : land_domainUG_id = land_domainUG_id_t(land_domainUG_id)
        if mode is not None : mode = mode_t(mode)
        
        _data_override_init.restype = None
        _data_override_init.argtypes = [ctypes.POINTER(atm_domain_id_t),
        ...
        _data_override_init(atm_domain_id,
                            ocn_domain_id,
                            ice_domain_id,
                            land_domain_id,
                            land_domainUG_id,
