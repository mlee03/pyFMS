...
class pyFMS_horiz_interp:
    ...
    def get_interp(
        ...
        if datatype is np.float64:
            ...
            area_src_t = np.ctypeslib.ndpointer(dtype=np.float64, ndim=2, shape=area_src_shape)
            area_dst_t = np.ctypeslib.ndpointer(dtype=np.float64, ndim=2, shape=area_dst_shape)
            ...
            src_dist_t = np.ctypeslib.ndpointer(dtype=np.float64, ndim=3, shape=src_dist_shape)
            rat_x_t = np.ctypeslib.ndpointer(dtype=np.float64, ndim=2, shape=rat_x_shape)
